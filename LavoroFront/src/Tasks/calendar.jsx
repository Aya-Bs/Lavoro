  ...
  const handleEventChange = async (changeInfo) => {
    ...
    try {
      ...
      const newEnd = changeInfo.event.end 
        ...
        : moment(newStart).add(1, "day").toDate();
      ...
      // 3. Validation conditionnelle pour les développeurs
      if (userRole === "Developer") {
        // Vérifier que les nouvelles dates sont bien dans l'intervalle start_date/deadline
        if (task.start_date && newStart < new Date(task.start_date)) {
          await Swal.fire({
            title: "Invalid Date",
            html: `Cannot set calendar date before the task start date (<strong>${new Date(task.start_date).toLocaleDateString()}</strong>)`,
            icon: "error",
            confirmButtonColor: "#3085d6",
          });
          changeInfo.revert();
        ...
        if (task.deadline && newEnd > new Date(task.deadline)) {
          await Swal.fire({
            title: "Invalid Date",
            html: `Cannot set calendar date after the task deadline (<strong>${new Date(task.deadline).toLocaleDateString()}</strong>)`,
            icon: "error",
            confirmButtonColor: "#3085d6",
          });
          changeInfo.revert();
        ...
        // Vérifier que la durée n'est pas négative
        if (newStart > newEnd) {
          await Swal.fire({
      ...
      await Swal.fire({
        title: "Error",
        text: error.response?.data?.error || "Failed to update dates",
        icon: "error",
        confirmButtonColor: "#3085d6",
      });
      changeInfo.revert();
