...
exports.getTeamById = async (req, res) => {
  ...
  /*exports.updateTeam = async (req, res) => {
      try {
          ...
          // Handle members array updates
          if (updates.members) {
              // Get current and new members
              const currentMembers = existingTeam.members.map(m => m.toString());
              const newMembers = [...new Set(updates.members.map(m => m.toString()))];

              // Ensure manager is included
              const managerId = existingTeam.manager_id.toString();
              if (!newMembers.includes(managerId)) {
                  newMembers.push(managerId);
              }
              ...
              if (addedMembers.length > 0) {
                  ...
                  const memberDocs = addedMembers.map(memberId => ({
                      team_id: id,
                      user_id: memberId,
                  ...
                  // Log each addition in TeamHistory
                  await Promise.all(addedMembers.map(async memberId => {
              ...
              if (removedMembers.length > 0) {
                  await TeamMember.deleteMany({
                      team_id: id,
                      user_id: { $in: removedMembers }
                  });

                  // Log each removal in TeamHistory
                  await Promise.all(removedMembers.map(async memberId => {
                      await TeamHistory.create({
                          team_id: id,
                          changed_by: userId,
